 ...
 *  \version		$rowid: mssql.lib.php,v 1.87 2011/08/04 13:58:14 eldy Exp $
 */

// Pour compatibilite lors de l'upgrade
if (! defined('SIEMP_DOCUMENT_ROOT'))	 define('SIEMP_DOCUMENT_ROOT', '../..');
if (! defined('ADODB_DATE_VERSION')) include_once(SIEMP_DOCUMENT_ROOT."/includes/adodbtime/adodb-time.inc.php");
...
/**
 *	\class      SiempDb
 *	\brief      Classe de gestion de la database de SIEMPRE
...
class SiempDb implements DB {
	 ...
	 */
	function SiempDb($type='oracle', $host, $user, $pass, $name='', $port=0)
	{
		global $conf,$langs;
		...
		if (! function_exists("mssql_connect"))
		{
			...
			Syslog::log("SiempDB::SiempDB : MSsql PHP functions for using MSsql driver are not available in this version of PHP",LOG_ERR);
			return $this->ok;
		}
		...
		if ($this->db)
		{
			// Si client connecte avec charset different de celui de la base SIEMPRE
			// (La base SIEMPRE a ete forcee en this->forcecharset a l'install)
			$this->connected = 1;
			$this->ok = 1;
		}
		else
		{
			...
			Syslog::log("SiempDB::SiempDB : Erreur Connect mssql_get_last_message=".$this->error,LOG_ERR);
		}
	...
	{
		Syslog::log("SiempDB::connect host=$host, port=$port, login=$login, passwd=--hidden--, name=$name");
		$newhost=$host;
		if ($port) $newhost.=':'.$port;
		$this->db  = @oci_connec($login, $passwd,$host);
		...
		// les nouvelles version de SIEMPRE car force par l'install SIEMPRE.
	...
	function query($query,$usesavepoint=0,$type='auto')
	{
		...
		$query = str_ireplace(", updated = updated", "", $query);
        ...
        if (! preg_match("/^COMMIT/i",$query) && ! preg_match("/^ROLLBACK/i",$query))
		{
			...
			if (! $ret)
			{
                ...
                Syslog::log("Mssql.lib::query SQL error: ".$query, LOG_WARNING);
			}
			$this->lastquery=$query;
			$this->results = $ret;
		}
        $ret = oci_parse($this->db, $query);
        $ret = @oci_execute($ret);
		return $ret;
	}
	...
	function fetch_object($resultset)
	{
		...
		return oci_fetch_object ($resultset);
	}
	...
	function fetch_array($resultset)
	{
		// Si le resultset n'est pas fourni, on prend le dernier utilise sur cette connexion
		if (! is_resource($resultset)) { $resultset=$this->results; }
		return oci_fetch_array($resultset);
	}
	...
	function fetch_row($resultset)
	{
		// Si le resultset n'est pas fourni, on prend le dernier utilise sur cette connexion
		if (! is_resource($resultset)) { $resultset=$this->results; }
		return @oci_fetch_row($resultset);
	}
	...
	function num_rows($resultset)
	{
		// Si le resultset n'est pas fourni, on prend le dernier utilise sur cette connexion
		if (! is_resource($resultset)) { $resultset=$this->results; }
		return oci_num_fields($resultset);
	}
	...
	function free($resultset=0)
	{
		...
		if (is_resource($resultset)) oci_free_statement($resultset);
	}
    ...
    function like($column, $value){
        ...
        if($column && $value){
            $return = " " . $column . " LIKE " . "'%" . $this->escape($value) . "%' ";
        }
	...
	function errno()
	{
		if (! $this->connected) {
			...
			// Constants to convert a MSSql error code to a generic SIEMPRE error code
			$errorcode_map = array(
	...
	{
		$this->results = $this->query('SELECT table_name FROM all_tables');
		return $this->results;
	}
