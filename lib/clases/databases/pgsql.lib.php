 ...
 *  \version	$rowid: pgsql.lib.php,v 1.111 2011/08/04 13:58:14 eldy Exp $
 */
...
require_once(SIEMP_DOCUMENT_ROOT."/core/class/syslog.class.php");
require_once(SIEMP_DOCUMENT_ROOT."/core/class/date.class.php");
require_once(SIEMP_DOCUMENT_ROOT."/core/class/db.interface.php");

/**
 * 	\class      SiempDb
 * 	\brief      Class to drive a Postgresql database for SIEMPRE
 */
class SiempDb implements DB{
     ...
     */
    function SiempDb($type = 'pgsql', $host, $user, $pass, $name = '', $port = 0) {
        global $conf, $langs;
        ...
        if (!function_exists("pg_connect")) {
            ...
            Syslog::log("SiempDB::SiempDB : Pgsql PHP functions are not available in this version of PHP", LOG_ERR);
            return $this->ok;
        }
        ...
        if (!$host) {
            $this->connected = 0;
            $this->ok = 0;
            $this->error = Translate::trans("ErrorWrongHostParameter");
            Syslog::log("SiempDB::SiempDB : Erreur Connect, wrong host parameters", LOG_ERR);
            return $this->ok;
        }
        ...
        if ($this->db) {
            ...
            Syslog::log("SiempDB::SiempDB : Erreur Connect " . $this->error, LOG_ERR);
        }
        ...
        if ($this->connected && $name) {
            if ($this->select_db($name)) {
                ...
                Syslog::log("SiempDB::SiempDB : Erreur Select_db " . $this->error, LOG_ERR);
            }
        } else {
    ...
    public function __destruct() {
        if ($this->db && $this->connected) {
            Syslog::log("SiempDB::__destruct",LOG_DEBUG);
            $this->connected = 0;
    ...
    function convertSQLFromMysql($line, $type = 'auto') {
        ...
        if ($line != "") {
            ...
            if ($type == 'dml') {
                ...
                # ALTER TABLE llx_SIEMPRE_modules ADD PRIMARY KEY pk_SIEMPRE_modules (numero, entity);
                ...
                # ALTER TABLE llx_SIEMPRE_modules DROP FOREIGN KEY fk_xxx;
    ...
    function getIntVersion() {
        ...
        if (String::strlen($vlist[1]) == 1) {
            $vlist[1] = "0" . $vlist[1];
        }
        if (String::strlen($vlist[2]) == 1) {
            $vlist[2] = "0" . $vlist[2];
        }
    ...
    function close() {
        if ($this->db) {
            Syslog::log("SiempDB::disconnect",LOG_DEBUG);
            $this->connected = 0;
            return pg_close($this->db);
        }
        return false;
    ...
    function begin() {
        if (!$this->transaction_opened) {
            ...
            if ($ret) {
                ...
                Syslog::log("BEGIN Transaction", LOG_DEBUG);
            }
            return $ret;
        } else {
            $this->transaction_opened++;
    ...
    function commit($log = '') {
        if ($this->transaction_opened <= 1) {
            ...
            if ($ret) {
                $this->transaction_opened = 0;
                Syslog::log("COMMIT Transaction", LOG_DEBUG);
            }
            return $ret;
        } else {
            $this->transaction_opened--;
    ...
    function rollback() {
        if ($this->transaction_opened <= 1) {
            ...
            $this->transaction_opened = 0;
            Syslog::log("ROLLBACK Transaction", LOG_DEBUG);
            return $ret;
        } else {
            $this->transaction_opened--;
            return 1;
    ...
    function query($query, $usesavepoint = 0, $type = 'auto') {
        ...
        while ($loop) {
            if (preg_match('/([^\'])([0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9] [0-9][0-9]:[0-9][0-9]:[0-9][0-9])/', $query)) {
                ...
                Syslog::log("Warning: Bad formed request converted into " . $query, LOG_WARNING);
            }
            else
                $loop = false;
        }
        ...
        if (!preg_match("/^COMMIT/i", $query) && !preg_match("/^ROLLBACK/i", $query)) { // Si requete utilisateur, on la sauvegarde ainsi que son resultset
            if (!$ret) {
                ...
                Syslog::log("Pgsql.lib::query SQL error: " . $query . " " . $this->lasterrno, LOG_WARNING);
                //print "\n>> ".$query."<br>\n";
    ...
    function like($column, $value){
        ...
        if($column && $value){
            $return = " " . $column . " ILIKE " . "'%" . $this->escape($value) . "%' ";
        }
     ...
     */
    function idate($string) {
        return adodb_strftime("%Y-%m-%d %H:%M:%S", $string);
    }
    ...
    function jdate($string) {
        ...
        $tmp = $string . '000000';
        $date = Date::mktime(substr($tmp, 8, 2), substr($tmp, 10, 2), substr($tmp, 12, 2), substr($tmp, 4, 2), substr($tmp, 6, 2), substr($tmp, 0, 4));
        return $date;
    }
    ...
    function errno() {
        if (!$this->connected) {
            ...
            if (preg_match('/duplicate/i', $errorlabel, $reg)) {
    ...
    function last_insert_id($tab, $fieldid = 'rowid') {
        ...
        //ECHO "SELECT currval('" . $tab . "_" . $fieldid . "_seq')";
    ...
    /**
     * 	\brief      Create a user
     * 	\param	    SIEMPRE_main_db_host 		Ip serveur
     * 	\param	    SIEMPRE_main_db_user 		Nom user a creer
     * 	\param	    SIEMPRE_main_db_pass 		Mot de passe user a creer
    ...
    function DDLCreateUser($SIEMPRE_main_db_host, $SIEMPRE_main_db_user, $SIEMPRE_main_db_pass, $SIEMPRE_main_db_name) {
        $sql = "create user \"" . addslashes($SIEMPRE_main_db_user) . "\" with password '" . addslashes($SIEMPRE_main_db_pass) . "'";

        Syslog::log("pgsql.lib::DDLCreateUser", LOG_DEBUG); // No sql to avoid password in log
    ...
    function getServerParametersValues($filter = '') {
        ...
        if ($filter)
            $resql.=" WHERE name = '" . addslashes($filter) . "'";
