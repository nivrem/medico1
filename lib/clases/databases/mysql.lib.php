 ...
 *  \brief      	Class file to manage SIEMPRE database access for a Mysql database
 *  \version		$Id: mysql.lib.php,v 1.121 2011/08/04 13:58:55 eldy Exp $
 */
// For compatibility during upgrade
if (!defined('SIEMP_DOCUMENT_ROOT'))
    define('SIEMP_DOCUMENT_ROOT', '../..');
if (!defined('ADODB_DATE_VERSION'))
    include_once(SIEMP_DOCUMENT_ROOT . "/includes/adodbtime/adodb-time.inc.php");

/**
 * 	\class      SiempDb
 * 	\brief      Class to manage SIEMPRE database access for a Mysql database
 */
class SiempDb {
     ...
     */
    function SiempDb($type = 'mysql', $host, $user, $pass, $name = '', $port = 0) {
        global $conf, $langs;

        if (!empty($conf->db->character_set))
            $this->forcecharset = $conf->db->character_set;
        if (!empty($conf->db->SIEMPRE_main_db_collation))
            $this->forcecollate = $conf->db->SIEMPRE_main_db_collation;
        ...
        if (!function_exists("mysql_connect")) {
            ...
            siemp_syslog("SiempDB::SiempDB : Mysql PHP functions for using Mysql driver are not available in this version of PHP. Try to use another driver.", LOG_ERR);
            return $this->ok;
        }
        ...
        if (!$host) {
            ...
            siemp_syslog("SiempDB::SiempDB : Erreur Connect, wrong host parameters", LOG_ERR);
            return $this->ok;
        }
        ...
        if ($this->db) {
            ...
            siemp_syslog("SiempDB::SiempDB : Erreur Connect mysql_error=" . $this->error, LOG_ERR);
        }
        ...
        if ($this->connected && $name) {
            if ($this->select_db($name)) {
                ...
                // If client connected with different charset than SIEMPRE HTML output
                $clientmustbe = '';
                ...
                siemp_syslog("SiempDB::SiempDB : Erreur Select_db " . $this->error, LOG_ERR);
            }
        } else {
    ...
    function select_db($database) {
        siemp_syslog("SiempDB::select_db database=" . $database, LOG_DEBUG);
    ...
    function connect($host, $login, $passwd, $name, $port = 0) {
        siemp_syslog("SiempDB::connect host=$host, port=$port, login=$login, passwd=--hidden--, name=$name", LOG_DEBUG);
    ...
    function getIntVersion() {
        ...
        if (siemp_strlen($vlist[1]) == 1) {
            $vlist[1] = "0" . $vlist[1];
        }
        if (siemp_strlen($vlist[2]) == 1) {
            $vlist[2] = "0" . $vlist[2];
        }
    ...
    function close() {
        if ($this->db) {
            //siemp_syslog("SiempDB::disconnect",LOG_DEBUG);
            $this->connected = 0;
    ...
    function begin() {
        if (!$this->transaction_opened) {
            ...
            if ($ret) {
                ...
                siemp_syslog("BEGIN Transaction", LOG_DEBUG);
            }
            return $ret;
        } else {
            $this->transaction_opened++;
    ...
    function commit($log = '') {
        if ($this->transaction_opened <= 1) {
            ...
            if ($ret) {
                $this->transaction_opened = 0;
                siemp_syslog("COMMIT Transaction" . ($log ? ' ' . $log : ''), LOG_DEBUG);
            }
            return $ret;
        } else {
            $this->transaction_opened--;
    ...
    function rollback($log = '') {
        if ($this->transaction_opened <= 1) {
            ...
            $this->transaction_opened = 0;
            siemp_syslog("ROLLBACK Transaction" . ($log ? ' ' . $log : ''), LOG_DEBUG);
            return $ret;
        } else {
            $this->transaction_opened--;
            return 1;
    ...
    function query($query, $usesavepoint = 0, $type = 'auto') {
        ...
        if (!preg_match("/^COMMIT/i", $query) && !preg_match("/^ROLLBACK/i", $query)) {
            ...
            if (!$ret) {
                ...
                siemp_syslog("Mysql.lib::query SQL error: " . $query . " " . $this->lasterrno, LOG_WARNING);
            }
    ...
    function jdate($string) {
        ...
        $tmp = $string . '000000';
        $date = siemp_mktime(substr($tmp, 8, 2), substr($tmp, 10, 2), substr($tmp, 12, 2), substr($tmp, 4, 2), substr($tmp, 6, 2), substr($tmp, 0, 4));
        return $date;
    }
    ...
    function errno() {
        if (!$this->connected) {
            ...
            // Constants to convert a MySql error code to a generic SIEMPRE error code
            $errorcode_map = array(
    ...
    function encrypt($fieldorvalue, $withQuotes = 0) {
        ...
        $cryptType = ($conf->db->SIEMPRE_main_db_encryption ? $conf->db->SIEMPRE_main_db_encryption : 0);

        //Encryption key
        $cryptKey = (!empty($conf->db->SIEMPRE_main_db_cryptkey) ? $conf->db->SIEMPRE_main_db_cryptkey : '');
    ...
    function DDLCreateDb($database, $charset = '', $collation = '', $owner = '') {
        ...
        // ALTER DATABASE SIEMPRE_db DEFAULT CHARACTER SET latin DEFAULT COLLATE latin1_swedish_ci
        $sql = 'CREATE DATABASE ' . $database;
        $sql.= ' DEFAULT CHARACTER SET ' . $charset . ' DEFAULT COLLATE ' . $collation;

        siemp_syslog($sql, LOG_DEBUG);
        $ret = $this->query($sql);
        if (!$ret) {
    ...
    function DDLDescTable($table, $field = "") {
        ...
        siemp_syslog(get_class($this) . "::DDLDescTable " . $sql, LOG_DEBUG);
    ...
    function DDLAddField($table, $field_name, $field_desc, $field_position = "") {
        ...
        siemp_syslog(get_class($this) . "::DDLAddField " . $sql, LOG_DEBUG);
        if (!$this->query($sql)) {
            return -1;
        } else {
            return 1;
    ...
    function DDLUpdateField($table, $field_name, $field_desc) {
        ...
        if ($field_desc['type'] == 'int' || $field_desc['type'] == 'varchar')
            $sql.="(" . $field_desc['value'] . ")";

        siemp_syslog(get_class($this) . "::DDLUpdateField " . $sql, LOG_DEBUG);
        if (!$this->query($sql))
            return -1;
        else
            return 1;
    ...
    function DDLDropField($table, $field_name) {
        ...
        siemp_syslog(get_class($this) . "::DDLDropField " . $sql, LOG_DEBUG);
        if (!$this->query($sql)) {
     ...
     * 	\param	    SIEMPRE_main_db_host 		Ip server
     * 	\param	    SIEMPRE_main_db_user 		Username to create
     * 	\param	    SIEMPRE_main_db_pass 		Password
     * 	\param		SIEMPRE_main_db_name		Database name where user must be granted
     * 	\return	    int							<0 if KO, >=0 if OK
     */
    function DDLCreateUser($SIEMPRE_main_db_host, $SIEMPRE_main_db_user, $SIEMPRE_main_db_pass, $SIEMPRE_main_db_name) {
        global $conf;
        $sql = "INSERT INTO user ";
        $sql.= "(Host,User,password,Select_priv,Insert_priv,Update_priv,Delete_priv,Create_priv,Drop_priv,Index_Priv,Alter_priv,Lock_tables_priv,entity)";
        $sql.= " VALUES ('" . addslashes($SIEMPRE_main_db_host) . "','" . addslashes($SIEMPRE_main_db_user) . "',password('" . addslashes($SIEMPRE_main_db_pass) . "')";
        $sql.= ",'Y','Y','Y','Y','Y','Y','Y','Y','Y',".$conf->entity.")";

        siemp_syslog("mysql.lib::DDLCreateUser", LOG_DEBUG); // No sql to avoid password in log
        $resql = $this->query($sql);
        if (!$resql) {
            siemp_syslog("mysqli.lib::DDLCreateUser sql=" . $sql, LOG_ERR);
            return -1;
        }

        $sql = "INSERT INTO db ";
        $sql.= "(Host,Db,User,Select_priv,Insert_priv,Update_priv,Delete_priv,Create_priv,Drop_priv,Index_Priv,Alter_priv,Lock_tables_priv,entity)";
        $sql.= " VALUES ('" . addslashes($SIEMPRE_main_db_host) . "','" . addslashes($SIEMPRE_main_db_name) . "','" . addslashes($SIEMPRE_main_db_user) . "'";
        $sql.= ",'Y','Y','Y','Y','Y','Y','Y','Y','Y',".$conf->entity.")";

        siemp_syslog("mysql.lib::DDLCreateUser sql=" . $sql, LOG_DEBUG);
        $resql = $this->query($sql);
        if (!$resql) {
            siemp_syslog("mysqli.lib::DDLCreateUser sql=" . $sql, LOG_ERR);
            return -1;
